...
export const LicenseDetailsModal: React.FC<LicenseDetailsModalProps> = ({ isOpen, onClose, license }) => {
  ...
  const daysRemaining = getDaysRemaining(license.expire_date);
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-start bg-gray-50/50">
          <div>
            ...
            <h2 className="text-2xl font-bold text-gray-900">{license.license_number}</h2>
            <p className="text-gray-500 mt-1 flex items-center gap-2">
              <Tag size={14} />
              {license.license_categories?.name || 'Uncategorized'}
            </p>
          </div>
          <button 
            onClick={onClose} 
              ...
              <div>
                ...
                <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 space-y-4">
                  <div>
                    ...
                    <p className="text-gray-800 font-medium">{license.standard_scope || '-'}</p>
                  </div>
                  <div>
                    <span className="text-xs text-blue-600/70 font-bold block mb-1">Criteria Scope</span>
                    <p className="text-gray-800">{license.criteria_scope || '-'}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="bg-white border-2 border-gray-100 p-5 rounded-xl space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-mono font-bold text-gray-900">{formatDate(license.expire_date)}</span>
                  </div>
                </div>
              </div>
